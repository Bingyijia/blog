...
## 1. jQuery概念
+ jQuery是一个JS的框架， 其实就是封装了一堆的JS函数，很好的兼容了不同的浏览器，更好用。
...
## 3.jQuery的入口函数
+ 3.1 语法
    * 方式1：jQuery(document).read(function(){  });
    * 方式2：$(function(){ });  两种都可以，而且可以添加多个
    * window.onload = function(){} 添加多个会覆盖
...
+ .blur()  //失去焦点事件，同上
+ .hover(handlerIn, handlerOut)  = .mouseenter(handlerIn).muoseleave(handlerOut)
  .hover(handlerInOut) = .on('mouseenter mouseleave', mouseInOut)
